        ...
        <div className="min-h-screen bg-gradient-to-b from-yellow-50/50 to-white">
            ...
            <section className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                {galleryImages.map((image, index) => (
                    <div
                    key={index}
                    className="group relative overflow-hidden rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 bg-gradient-to-br from-white/90 to-yellow-50/50 cursor-pointer"
                    onClick={() => openImageModal(image)}
                    >
                    <img
                        src={image.src}
                        alt={image.alt}
                        className="w-full h-64 md:h-72 lg:h-80 object-cover group-hover:scale-102 transition-transform duration-700"
                    />
                ...
                <button className="px-12 py-4 bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-600 hover:to-green-700 text-white font-bold rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 text-lg">
                    Load More Photos
                </button>
                </div>
            </section>
            ...
            {isModalOpen && (
                ...
                onClick={closeModal}
                >
                <div className="relative max-w-3xl w-full max-h-[90vh] mx-auto animate-fadeIn">

                    {/* Main Image */}
                    <img
                    src={selectedImage?.src}
                    alt={selectedImage?.alt}
                    className="w-full h-auto max-h-[85vh] object-contain rounded-2xl mx-auto block"
                    />
