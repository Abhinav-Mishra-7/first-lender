...
import { CheckCircle2, Send, Briefcase, Sparkles, RefreshCw, Smartphone, Mail, DollarSign } from 'lucide-react';
import axiosClient from "../../../utils/axiosClient";

export default function BusinessLoanForm({ onSubmit, recipientEmail = 'abhinavmishra0193@gmail.com' }) {
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      newErrors.name = 'Business or promoter name is required';
    }
    ...
    if (!formData.monthlyTurnover.trim()) {
      newErrors.monthlyTurnover = 'Monthly turnover is required';
    } else if (isNaN(Number(formData.monthlyTurnover.replace(/,/g, '')))) {
      newErrors.monthlyTurnover = 'Please enter a valid amount';
    }
  ...
  const handleTurnoverChange = (e) => {
    ...
    setFormData((prev) => ({ ...prev, monthlyTurnover: formatted }));
    if (errors.monthlyTurnover) {
      setErrors((prev) => ({ ...prev, monthlyTurnover: undefined }));
    }
  };
  ...
  return (
    ...
    <motion.div 
      ...
      id="business-loan-form-container"
    >
      ...
      <div className="px-5 sm:px-8 pb-6 sm:pb-8 pt-6 sm:pt-8" id="business-loan-body">
        <AnimatePresence mode="wait">
          {submitStatus?.type === 'success' ? (
            ...
            >
              ...
              <div className="bg-neutral-50/50 rounded-2xl p-4 sm:p-6 text-left border border-neutral-100/60 max-w-xl mx-auto space-y-2.5 sm:space-y-3.5 shadow-sm text-xs sm:text-sm">
                <div className="flex justify-between items-center py-2 border-b border-neutral-200/50">
                  <span className="text-neutral-500 text-[12px] font-extrabold uppercase tracking-wider">Business Name:</span>
                ...
                <div className="flex justify-between items-center py-2 border-b border-neutral-200/50">
                  <span className="text-neutral-500 text-[12px] font-extrabold uppercase tracking-wider">Monthly Turnover:</span>
                  <span className="text-amber-600 font-extrabold text-sm sm:text-base text-right">₹{formData.monthlyTurnover}</span>
                </div>
                {formData.message && (
                  <div className="pt-2">
                    <span className="text-neutral-500 text-[12px] font-extrabold uppercase tracking-wider block mb-1">Company Details:</span>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6" id="business-form-element" noValidate>
              {submitStatus?.type === 'error' && (
                <div className="p-4 bg-red-50 border border-red-200 text-red-600 text-xs font-bold uppercase rounded-xl tracking-tight" id="business-form-error-alert">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div className="space-y-1.5" id="business-field-name">
                ...
                {/* Mobile No */}
                <div className="space-y-1.5" id="business-field-mobile">
                  <div className="relative">
                    <input
                      type="tel"
                      id="b-mobile"
              ...
              {/* Grid layout for Email and turnover */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                {/* Email Address */}
                <div className="space-y-1.5" id="business-field-email">
                  <div className="relative">
                    <input
                      type="email"
                      id="b-email"
                ...
                {/* Turnover */}
                <div className="space-y-1.5" id="business-field-turnover">
                  <div className="relative">
                    <input
                      type="text"
                      id="b-turnover"
                      ...
                      placeholder="Monthly Business Turnover *"
                      className={`block w-full px-4 sm:px-5 py-3 sm:py-4 text-xs sm:text-sm rounded-xl sm:rounded-[1.2rem] bg-white text-neutral-800 placeholder-neutral-400 focus:outline-none transition-all duration-300 shadow-lg hover:shadow-[0_10px_24px_rgba(0,0,0,0.035)] focus:shadow-[0_12px_28px_rgba(0,0,0,0.055)] border focus:border-amber-400
                  ...
                  {errors.monthlyTurnover && <p className="text-[9px] sm:text-[10px] text-red-500 mt-1 pl-2 font-bold uppercase tracking-tight">{errors.monthlyTurnover}</p>}
                </div>
              </div>

              {/* Message */}
              <div className="space-y-1.5" id="business-field-message">
                <div className="relative">
                  <textarea
                    id="b-message"
                    name="message"
              ...
              <div className="pt-2 flex justify-center" id="business-submit-section">
                <motion.button
