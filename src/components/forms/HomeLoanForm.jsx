...
import { CheckCircle2, Send, Home, Sparkles, RefreshCw, Smartphone, Mail, DollarSign } from 'lucide-react';
import axiosClient from "../../../utils/axiosClient";

export default function HomeLoanForm({ onSubmit, recipientEmail = 'abhinavmishra0193@gmail.com' }) {
  ...
  const validateForm = () => {
    ...
    if (!formData.assetValue.trim()) {
      newErrors.assetValue = 'Estimated asset value is required';
    } else if (isNaN(Number(formData.assetValue.replace(/,/g, '')))) {
      newErrors.assetValue = 'Please enter a valid amount';
    }

    if (!formData.monthlyIncome.trim()) {
      newErrors.monthlyIncome = 'Monthly income is required';
    } else if (isNaN(Number(formData.monthlyIncome.replace(/,/g, '')))) {
  ...
  const handleAssetValueChange = (e) => {
    ...
    setFormData((prev) => ({ ...prev, assetValue: formatted }));
    if (errors.assetValue) {
      setErrors((prev) => ({ ...prev, assetValue: undefined }));
    }
  };
  ...
  const handleIncomeChange = (e) => {
    const rawVal = e.target.value.replace(/[^0-9]/g, '');
    const formatted = rawVal ? Number(rawVal).toLocaleString('en-IN') : '';
    setFormData((prev) => ({ ...prev, monthlyIncome: formatted }));
  ...
  return (
    ...
    <motion.div 
      ...
      className="w-full max-w-6xl pt-3 pb-3 mx-auto bg-yellow-100 rounded-2xl sm:rounded-2xl shadow-lg border border-neutral-100/50 overflow-hidden hover:shadow-xl hover:shadow-blue-200 transition-all duration-400" 
      id="asset-loan-form-container"
    >
      ...
      <div className="px-5 sm:px-8 pb-6 sm:pb-8 pt-6 sm:pt-8" id="asset-loan-body">
        <AnimatePresence mode="wait">
          {submitStatus?.type === 'success' ? (
            <motion.div 
              ...
              className="text-center space-y-4 sm:space-y-3" 
              id="asset-success-message"
            >
              ...
              <div className="bg-neutral-50/50 rounded-2xl p-4 sm:p-6 text-left border border-neutral-100/60 max-w-xl mx-auto space-y-2.5 sm:space-y-3.5 shadow-sm text-xs sm:text-sm">
                <div className="flex justify-between items-center py-2 border-b border-neutral-200/50">
                  <span className="text-neutral-500 text-[12px] font-extrabold uppercase tracking-wider">Applicant Name:</span>
                ...
                <div className="flex justify-between items-center py-2 border-b border-neutral-200/50">
                  <span className="text-neutral-500 text-[12px] font-extrabold uppercase tracking-wider">Property / Asset Value:</span>
                  <span className="text-neutral-800 font-black text-right">₹{formData.assetValue}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-neutral-200/50">
                  <span className="text-neutral-500 text-[12px] font-extrabold uppercase tracking-wider">Monthly Income:</span>
                  <span className="text-amber-600 font-extrabold text-sm sm:text-base text-right">₹{formData.monthlyIncome}</span>
                </div>
                {formData.message && (
                  <div className="pt-2">
                    <span className="text-neutral-500 text-[12px] font-extrabold uppercase tracking-wider block mb-1">Asset Details:</span>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6" id="asset-form-element" noValidate>
              {submitStatus?.type === 'error' && (
                <div className="p-4 bg-red-50 border border-red-200 text-red-600 text-xs font-bold uppercase rounded-xl tracking-tight" id="asset-form-error-alert">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                {/* Applicant Name */}
                <div className="space-y-1.5" id="asset-field-name">
                ...
                {/* Mobile No */}
                <div className="space-y-1.5" id="asset-field-mobile">
                  <div className="relative">
                    <input
                      type="tel"
                      id="a-mobile"
              ...
              {/* Grid layout for Email and Purpose Selection label */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                {/* Email Address */}
                <div className="space-y-1.5" id="asset-field-email">
                  <div className="relative">
                    <input
                      type="email"
                      id="a-email"
              ...
              {/* Grid layout for Asset value & Monthly Income */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                {/* Asset Value */}
                <div className="space-y-1.5" id="asset-field-val">
                  <div className="relative">
                    <input
                      type="text"
                      id="a-value"
                      ...
                      placeholder="Loan Amount (Asset Value INR) *"
                      className={`block w-full px-4 sm:px-5 py-3 sm:py-4 text-xs sm:text-sm rounded-xl sm:rounded-[1.2rem] bg-white text-neutral-800 placeholder-neutral-400 focus:outline-none transition-all duration-300 shadow-lg hover:shadow-[0_10px_24px_rgba(0,0,0,0.035)] focus:shadow-[0_12px_28px_rgba(0,0,0,0.055)] border focus:border-amber-400
                        ${errors.assetValue 
                          ? 'border-red-400 bg-red-50/10' 
                          : 'border-neutral-100/85'
                  ...
                  {errors.assetValue && <p className="text-[9px] sm:text-[10px] text-red-500 mt-1 pl-2 font-bold uppercase tracking-tight">{errors.assetValue}</p>}
                </div>

                {/* Monthly Income */}
                <div className="space-y-1.5" id="asset-field-income">
                  <div className="relative">
                    <input
                      type="text"
                      id="a-income"
                  ...
                  {errors.monthlyIncome && <p className="text-[9px] sm:text-[10px] text-red-500 mt-1 pl-2 font-bold uppercase tracking-tight">{errors.monthlyIncome}</p>}
                </div>
              </div>

              {/* Message */}
              <div className="space-y-1.5" id="asset-field-message">
                <div className="relative">
                  <textarea
                    id="a-message shadow text-xs sm:text-sm"
                    name="message"
              ...
              <div className="pt-2 flex justify-center" id="asset-submit-section">
                <motion.button
